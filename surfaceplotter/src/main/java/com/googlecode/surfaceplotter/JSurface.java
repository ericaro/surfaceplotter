...
/**
 * The class <code>JSurface</code> is responsible
 * for the generation of surface images and user mouse events handling. 
 *
 * @author  Yanto Suryono
 */

public class JSurface extends javax.swing.JPanel
{
	...
	private VolatileImage buffer; // the buffer used to draw image (in in rotation process
  ...
  /**
	...
	public JSurface(SurfaceModel model) {
		...
		MyProperty myp= new MyProperty();
		model.addPropertyChangeListener(myp);
		model.addChangeListener(myp);
	...
	class MyProperty implements PropertyChangeListener, javax.swing.event.ChangeListener
	{
	...
	private void init()
	{
		...
		colors.setPlotColor(plot_color);
		colors.setPlotType(plot_type);
	...
	public void doExportSVG(File file)
	{
			if (file==null) return;
	...
	public void paintComponent(Graphics g) 
	{
		...
		{
			//model.setMessage("New image size: " + getBounds().width + "x" + getBounds().height);
	...
	private void renderOffscreen() {
		do {
				...
				buffer.validate(getGraphicsConfiguration()) ==VolatileImage.IMAGE_INCOMPATIBLE)
			{
				...
				buffer = createVolatileImage(getBounds().width, getBounds().height);
			}
	...
	*/
	private synchronized void draw(Graphics graphics)
	{
		...
		switch(plot_type)
		{
		case SurfaceModel.PLOT_TYPE_CONTOUR:plotContour();break;
		case SurfaceModel.PLOT_TYPE_DENSITY:plotDensity();break;
		case SurfaceModel.PLOT_TYPE_WIREFRAME:plotWireframe();break;
		case SurfaceModel.PLOT_TYPE_SURFACE:plotSurface();break;
		}
		cleanUpMemory();
	}
	...
	private final void plotPlane(SurfaceVertex[] vertex, int verticescount) {
		...
		if (count > 0) {
			...
			if (isMesh || (plot_color == SurfaceModel.PLOT_COLOR_OPAQUE)) 
			{// the OPAQUE COLOR MODE doesn't show the shape, so it must be drawn with mesh
	...
	private final void splitPlotPlane(SurfaceVertex[] values1, SurfaceVertex[] values2) { 
				...
				if (trackposition == COINCIDE) {
					...
					if (trackposition != UPPER) {      
						...
						upperpart[uppercount++] = 
							lowerpart[lowercount++] = new SurfaceVertex(projector,xi,yi,zi);
						upperpart[uppercount++] = values2[i];                    
					...
					if (trackposition == COINCIDE) {
						...
						if (trackposition != LOWER) {      
							...
							lowerpart[lowercount++] = 
								upperpart[uppercount++] = new SurfaceVertex(projector,xi,yi,zi);
							lowerpart[lowercount++] = values2[i];
		...
		*/
			...
			// there is a problem in drawing two curves in the same draw only for
			...
			if (lowercount < 3) {
				...
				if (uppercount < 3) {
					...
					if (upper_first) {
						curve=2;//color = dualshadeColorSecondHue;//dualshadeColorFirstHue;;
						plotPlane(upperpart,uppercount);   
						curve=1;//color = dualshadeColorFirstHue;
						plotPlane(values1,4);
						curve=2;//color = dualshadeColorSecondHue;//dualshadeColorFirstHue;;
			...
			while (lx != end_lx) {
				...
				if (!plotboth) {          
					...
					if (plottable(values1)) {
						...
						{curve=1;plotPlane(values1,4);}
					}
					else {
						if (plottable(values2)) {curve=2;plotPlane(values2,4);}
					}
				}
				lx += sx; 
			}
	...
	private final void plotSurface() {
		...
		cop = new SurfaceVertex(projector,distance * projector.getSinRotationAngle(), 
		...
		if (isDisplayXY || isDisplayGrids) {    
			...
			for (int i=-10; i <= 10; i++) {
				...
				if (!isDisplayGrids || (i % (t_x / 2) == 0) || isDisplayXY) {
					...
					if ((isDisplayGrids) && (i % t_x == 0)) {
						graphics.drawLine(xc,contourConvertY(-10.0f),
					...
					if ((i % t_x == 0) && isDisplayXY) {
						outFloat(graphics,xc,contourConvertY(-10.7f),
							(float)((double)(i+10)/20*(xmax-xmin)+xmin),
							Label.CENTER,TOP);
					}        
				}          
				if (isDisplayXY) {
					outString(graphics,(x1+x2)/2,
	...
	private final void createContour() {
		...
		for (int counter = 0; counter <= contour_lines+1; counter++) {    
			...
			if ((plot_type != SurfaceModel.PLOT_TYPE_WIREFRAME) &&
				...
				if (ok(contour_x,contour_n)&&ok(contour_y,contour_n)) graphics.fillPolygon(contour_x,contour_y,contour_n);
			}
